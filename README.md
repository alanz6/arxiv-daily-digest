...
Claude is **also** the runtime model for the application itself in the sense that an LLM does the paper scoring, summarization, and trend clustering — but at runtime the LLM is selected via the `DO_INFERENCE_MODEL` environment variable and runs on DigitalOcean's hosted infrastructure (typically `openai-gpt-oss-20b` for cost/speed). The Anthropic API is not called at runtime.
